...
static esp_err_t capturar_com_flash_handler(httpd_req_t *req) {
  camera_fb_t *fb = NULL;
  ...
  // Aguarda 150ms para o sensor da câmera se ajustar à luz
  vTaskDelay(150 / portTICK_PERIOD_MS); 

  // Captura a imagem com o flash ligado
  fb = esp_camera_fb_get();

  digitalWrite(4, LOW);

  if (!fb) {
    log_e("Camera flash capture failed");
  ...
  if (fb->format == PIXFORMAT_JPEG) {
    ...
    httpd_resp_send_chunk(req, NULL, 0); // Finaliza o envio por partes
  }
...
void startCameraServer() {
  ...
  httpd_uri_t capturar_com_flash_uri = {
    .uri = "/capturar_com_flash",
    .method = HTTP_GET,
    .handler = capturar_com_flash_handler,
    .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &capturar_com_flash_uri);
